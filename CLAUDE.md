...
**In progress (started 2026-07-31, not yet committed):** Spanish/English language toggle for the driver flow. Scope decided with the user: the toggle lives on the Welcome screen, but the chosen language persists and applies across the *entire* driver flow (Welcome → vehicle/driver/odometer → all 6 wizard steps → signature → submission complete) — not just the landing page. Admin/manager screens stay English-only (different audience). Planned approach: a `LanguageContext` (React context + localStorage persistence, pattern-matched to the existing `InspectionContext`) plus a `src/lib/translations.ts` dictionary of `en`/`es` strings keyed by UI element; inspection step titles/descriptions move out of `src/data/inspectionSteps.ts` and into the translation dictionary (keyed by step `id`) instead of being hardcoded English there. Vehicle labels (Truck #101, etc.) and quick-select driver names are treated as proper nouns and left untranslated. None of this has been written to files yet — start from scratch next session unless work resumes within this one.
